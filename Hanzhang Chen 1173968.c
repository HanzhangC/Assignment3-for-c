/* Program to generate term-biased snippets for paragraphs of text.
   ...
   Signed by: Hanzhang Chen 1173968
   Dated:     01/09/2022
...
*/
typedef char para_t[MAX_PARA_LEN+1];
typedef char match_t[MAX_PARA_LEN+1];
...
main(int argc, char *argv[]) {
    ...
    while ((len=get_paragraph(p, MAX_PARA_LEN)) != EOF){
        para += 1;
...
get_word(word_t W, int limit) {
    ...
    while ((c=getchar())!=EOF && !isalnum(c)) {
        /* see whether it's a para-end */
        if (strchr(PARA_SEPARATOR, c) && (limit>0)){
            if (strchr(PARA_SEPARATOR, (c=getchar()))){
                return PARA_END;
            }
        }
    }
...
get_paragraph(para_t P, int limit){
    ...
    while ((get_word(w, MAX_WORD_LEN) != PARA_END) && (limit>0)){
        if (cont+strlen(w)<=72){
            strcat(P, w);
            strcat(P, " ");
            cont = cont + strlen(w) + 1;
            len += 1;
            limit -= 1;
        }else {
            strcat(P, "\n");
